...
class MetroGraph
{
    ...
    static void Main()
    {
        ...
        TrouverTempsEntreStations("Porte de la Villette", "Trocadero");
        int indexDepart = -1;
        int indexArrivee = -1;
        for (int i = 0; i < nomsStations.Count; i++)
        {
            if (nomsStations[i] == "Porte de la Villette")
                indexDepart = i;

            if (nomsStations[i] == "Trocadero")
                indexArrivee = i;
    ...
    public static void LireCoordonneesCSV(string cheminCSV)
    {
        ...
        using (StreamReader sr = new StreamReader(cheminCSV))
        {
            ...
            while ((ligne = sr.ReadLine()) != null)
            {
                ...
                float lon = float.Parse(morceaux[1], CultureInfo.InvariantCulture);  // L'IA m'a dit de faire CultureInfo.InvariantCulture pour garantir le . comme séparateur décimal car j'arrivais pas autrement
    ...
    public static void TrouverTempsEntreStations(string nomDepart, string nomArrivee)
    {
        int indexDepart = -1;
        int indexArrivee = -1;
        // Recherche manuelle des indices des stations dans la liste
        for (int i = 0; i < nomsStations.Count; i++)
        {
        ...
        //Temps avec les trois algorithmes
        ...
        //Chemin et détection des changements
        List<int> chemin = DijkstraChemin(indexDepart, indexArrivee);
        int tempsTotal = 0;
